...
export function SupplierDetail({ supplierId }: SupplierDetailProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#fbfcfd]">
              ...
              <div className="space-y-1">
                ...
                <div className="space-y-3">
                  <div className="h-12 w-full px-4 rounded-xl bg-gray-50 border border-gray-100 flex items-center gap-3">
                    ...
                    <span className="font-bold text-sm text-foreground/80 truncate">{supplier.email || "No email documented"}</span>
                  </div>
                  <div className="h-12 w-full px-4 rounded-xl bg-gray-50 border border-gray-100 flex items-center gap-3">
            ...
            <CardHeader className="bg-emerald-50/50 border-b border-emerald-100/50 py-5">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Real-time stats</span>
                </div>
              </div>
            </CardHeader>
                        ...
                        <div className="p-5 rounded-2xl bg-gray-50/50 border border-gray-100 group-hover:bg-white group-hover:shadow-md transition-all duration-300">
                          ...
                          {event.relatedId && (
                            <div className="flex items-center gap-2 pt-3 border-t border-gray-100/50">
                              ...
                              <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/50">Ref: {event.relatedId}</span>
                            </div>
                          )}
                        </div>
                      </div>
            ...
            <TabsContent value="orders" className="p-8 mt-0 outline-none">
              {loadingOrders ? (
                <div className="py-20 text-center italic">
                   <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 animate-pulse">Fetching procurement history...</p>
                </div>
              ) : orders.length > 0 ? (
                <div className="bg-white rounded-3xl border border-gray-100 shadow-xl shadow-gray-200/50 overflow-hidden">
                  <Table>
                    <TableHeader className="bg-gray-50/50 border-b border-gray-100">
                      <TableRow>
                        <TableHead className="text-[10px] font-black uppercase tracking-widest py-4 pl-6">Order Registry</TableHead>
                        <TableHead className="text-[10px] font-black uppercase tracking-widest py-4">Total Value</TableHead>
                        <TableHead className="text-[10px] font-black uppercase tracking-widest py-4">Settlement Progress</TableHead>
                        <TableHead className="text-[10px] font-black uppercase tracking-widest py-4">Fulfillment Status</TableHead>
                        <TableHead className="text-right text-[10px] font-black uppercase tracking-widest py-4 pr-6">Directives</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {orders.map((order) => (
                        <TableRow key={order.id} className="group hover:bg-muted/30 transition-colors">
                          <TableCell className="py-5 pl-6">
                            <div className="flex flex-col">
                              ...
                              <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-1 opacity-60">{order.created_at.split('T')[0]}</span>
                            </div>
                          </TableCell>
                          ...
                          <TableCell className="py-5">
                            <div className="flex flex-col gap-1.5">
                              <div className="flex items-center gap-2 font-black text-[11px] text-foreground/80">
                                <span className="text-emerald-600">{formatCurrency(order.paid_amount)}</span>
                                <span className="text-muted-foreground/30">/ {formatCurrency(order.total_amount)}</span>
                              </div>
                            </div>
                          </TableCell>
                          <TableCell className="py-5">
          ...
          <div className="bg-white rounded-[2.5rem] w-full max-w-lg max-h-[90vh] flex flex-col shadow-[0_32px_128px_-32px_rgba(0,0,0,0.5)] overflow-hidden scale-in-center">
            <div className="flex justify-between items-center p-8 border-b border-gray-50">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-xl font-black tracking-tight">Modify Entity Credentials</h3>
              </div>
      ...
      {showPaymentModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 z-[100] animate-in fade-in duration-300">
           <div className="bg-white rounded-[2.5rem] w-full max-w-lg shadow-[0_32px_128px_-32px_rgba(0,0,0,0.5)] overflow-hidden scale-in-center">
