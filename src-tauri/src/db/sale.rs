...
pub fn update_sale(sale: Sale) -> Result<(), String> {
    ...
    let (old_client_id, old_total_amount, old_status): (String, f64, String) = conn.query_row(
    ...
    if old_status == "completed" && sale.status != "completed" {
        adjust_client_balance_internal(&conn, &old_client_id, -old_total_amount, "Sale Reverted to Draft", &sale.sale_number)?;
    } else if old_status != "completed" && sale.status == "completed" {
        adjust_client_balance_internal(&conn, &sale.client_id, old_total_amount, "Sale Completed", &sale.sale_number)?;
    } else if old_status == "completed" && sale.status == "completed" {
        if old_client_id != sale.client_id {
            adjust_client_balance_internal(&conn, &old_client_id, -old_total_amount, "Sale Moved to Another Client", &sale.sale_number)?;
            adjust_client_balance_internal(&conn, &sale.client_id, old_total_amount, "Sale Moved from Another Client", &sale.sale_number)?;
        }
    }

    recalculate_sale_total(&sale.id)?;
    recalculate_sale_payment_status(&sale.id)?;
    ...
    conn.execute(
        ...
        params![
            ...
            chrono::Utc::now().to_rfc3339(),
            "updated",
            format!("Sale {} updated", sale.sale_number),
            sale.created_by,
        ],
    )
    .map_err(|e| e.to_string())?;
...
fn adjust_client_balance_internal(conn: &rusqlite::Connection, client_id: &str, amount: f64, note_prefix: &str, sale_num: &str) -> Result<(), String> {
    if amount.abs() < 0.001 {
        return Ok(());
    }
    ...
    conn.execute(
        ...
        params![
            ...
            chrono::Utc::now().to_rfc3339(),
            "Sale Created",
            format!("{}: Sale {}", note_prefix, sale_num),
            amount,
        ],
    ).map_err(|e| e.to_string())?;
...
pub fn add_sale_item(item: SaleItem) -> Result<(), String> {
    ...
    conn.execute(
        "INSERT INTO sale_items (id, sale_id, item_id, item_name, quantity, unit_price, total_price, notes)
    ...
    if sale_status == "completed" {
        if let Some(item_id) = &item.item_id {
            ...
            let sale_num: String = conn.query_row(
            ...
            conn.execute(
                ...
                params![
                    ...
                    "Sold",
                    -item.quantity,
                    format!("Added item to completed sale {}", sale_num),
                    item.sale_id,
                ],
            ).map_err(|e| e.to_string())?;
        }
...
pub fn update_sale_item(item: SaleItem) -> Result<(), String> {
    ...
    let old_item: Option<(Option<String>, i32)> = conn.query_row(
        "SELECT i.item_id, i.quantity FROM sale_items i JOIN customer_sales o ON i.sale_id = o.id WHERE i.id = ?1 AND o.status = 'completed'",
        params![item.id],
        ...
        if let Some(new_id) = &item.item_id {
            ...
            conn.execute(
                ...
                params![
                    ...
                    chrono::Utc::now().to_rfc3339(),
                    "Adjustment",
                    old_qty as i64 - item.quantity as i64,
...
pub fn remove_sale_item(item_id: String, sale_id: String) -> Result<(), String> {
    ...
    let item_info: Option<(String, Option<String>, i32, String)> = conn.query_row(
        "SELECT i.item_name, i.item_id, i.quantity, o.status FROM sale_items i JOIN customer_sales o ON i.sale_id = o.id WHERE i.id = ?1",
        params![item_id],
...
fn recalculate_sale_total(sale_id: &str) -> Result<(), String> {
    ...
    let (old_total, status, client_id, sale_num): (f64, String, String, String) = conn.query_row(
    ...
    recalculate_sale_payment_status(sale_id)?;
...
pub fn add_sale_payment(payment: SalePayment) -> Result<(), String> {
    ...
    conn.execute(
        "INSERT INTO sale_payments (id, sale_id, amount, method, date, received_by, notes)
         VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)",
        params![
            payment.id,
            payment.sale_id,
    ...
    recalculate_sale_payment_status(&payment.sale_id)?;
    
    let (client_id, sale_number): (String, String) = conn.query_row(
    ...
    conn.execute(
        ...
        params![
            ...
            chrono::Utc::now().to_rfc3339(),
            "payment_added",
            format!("Payment of ${:.2} received", payment.amount),
            payment.received_by,
        ],
    )
    .map_err(|e| e.to_string())?;
...
fn recalculate_sale_payment_status(sale_id: &str) -> Result<(), String> {
    let conn = db::get_connection().map_err(|e| e.to_string())?;
    ...
    let (total_amount, paid_amount): (f64, f64) = conn
        .query_row(
            "SELECT 
             ...
             FROM customer_sales o
             LEFT JOIN sale_payments p ON o.id = p.sale_id
             WHERE o.id = ?1
             GROUP BY o.id",
            params![sale_id],
            |row| Ok((row.get(0)?, row.get(1)?)),
...
pub fn complete_sale(sale_id: String) -> Result<(), String> {
    ...
    let (sale_number, status, client_id, total_amount): (String, String, String, f64) = conn
        .query_row(
            "SELECT sale_number, status, client_id, total_amount FROM customer_sales WHERE id = ?1",
            params![sale_id],
            |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?, row.get(3)?)),
        )
        .map_err(|e| e.to_string())?;
        ...
        if let Some(item_id) = item_id_opt {
            ...
            conn.execute(
                ...
                params![
                    ...
                    "Sold",
                    -quantity,
                    format!("Sold in sale {}", sale_number),
                    sale_id,
                ],
            )
            .map_err(|e| e.to_string())?;
    ...
    conn.execute(
        "UPDATE customer_sales SET status = 'completed', updated_at = ?2 WHERE id = ?1",
        params![sale_id, chrono::Utc::now().to_rfc3339()],
    )
    .map_err(|e| e.to_string())?;
    ...
    conn.execute(
        ...
        params![
            ...
            chrono::Utc::now().to_rfc3339(),
            "Sale Created",
            format!("Sale {} completed", sale_number),
            total_amount,
        ],
    ).map_err(|e| e.to_string())?;
