...
export function RepairDetail({ repair, onClose }: RepairDetailProps) {
  const handlePrint = () => {
    ...
    if (printContent) {
      ...
      if (printWindow) {
        ...
        printWindow.document.write(
          "<style> @media print { body { -webkit-print-color-adjust: exact; print-color-adjust: exact; padding: 20px; } #print-button, .no-print { display: none !important; } .print-only-block { display: block !important; } .print-table { width: 100%; border-collapse: collapse; margin-top: 10px; } .print-table th, .print-table td { border: 1px solid #ccc; padding: 8px; text-align: left; } .print-table th { background-color: #f2f2f2; } } </style>"
        );
  ...
  const totalPartsCost = useMemo(() => {
    ...
    return repair.usedParts.reduce(
    ...
    <Dialog open={!!repair} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[725px]">
        <div id="receipt-content" className="print-only-block hidden">
          ...
          <Separator className="my-2" />
          <p>
            <strong>Device:</strong> {repair.deviceBrand} {repair.deviceModel}
          </p>
          <p>
          ...
          <p>
            ...
            {format(new Date(repair.dateReceived), "PPP p")}
          </p>
          <p>
            <strong>Repair Status:</strong> {repair.repairStatus}
          </p>
          <p>
            <strong>Payment Status:</strong> {repair.paymentStatus}
          </p>{" "}
          {/* Added Payment Status for print */}
          {repair.usedParts && repair.usedParts.length > 0 && (
            <>
              ...
              <p className="mt-1">
                <strong>Total Parts Cost:</strong> ${totalPartsCost.toFixed(2)}
              </p>
            </>
          )}
          <Separator className="my-2" />
        ...
        <div className="no-print">
          ...
          <ScrollArea className="max-h-[70vh] pr-6">
            <div className="grid gap-4 py-4">
              ...
              <Separator />
              <DetailItem
                label="Quoted/Estimated Cost"
                value={`$${estimatedCostValue.toFixed(2)}`}
              />
              <DetailItem
                label="Date Received"
                value={format(new Date(repair.dateReceived), "PPP p")}
              />
              <DetailItem
                label="Current Repair Status"
                value={repair.repairStatus}
              />
              <DetailItem label="Payment Status" value={repair.paymentStatus} />{" "}
              {/* Added Payment Status for display */}
              {repair.usedParts && repair.usedParts.length > 0 && (
                <>
                  <Separator />
                  ...
                  <div className="text-right font-semibold text-primary">
                    Estimated Profit (Excl. Labor): $
                    {profitExcludingParts.toFixed(2)}
                  </div>
                </>
              )}
